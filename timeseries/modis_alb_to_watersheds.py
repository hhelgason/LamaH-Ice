...
def modis_to_masked_xr(ds, geo, date, mask, variable_name):
    ...
    ds_new = xr.Dataset(
        data_vars=dict(R_median_filter_albe_GFD=(["x", "y", "time"], ds[variable_name].values[..., np.newaxis])), #'R_median_fsca'
        coords=dict(
        ...
        attrs=dict(description='MODIS glacier albedo'),)
    
    # Now we mask the dataset
    ds_masked = ds_new[variable_name].where(mask)
    return(ds_masked)
...
save_folder = '/home/xhordurbhe/Verkefni/lamah_ice/modis_on_watersheds/data_albedo_glaciers_final'

path = '/home/xhordurbhe/Verkefni/lamah_ice/data/final_shapefiles/glaciers_in_catchments_wgs84.shp'
...
variable_name = 'R_median_filter_albe_GFD'
date = '2016-04-01'
...
mask_path = '/home/xhordurbhe/Verkefni/lamah_ice/modis_on_watersheds/regionmask_glaciers_mask_final.p'
...
if __name__ == '__main__':
    ...
    filtered_filenames = [filename for filename in filenames if '2023' not in filename]    
    filtered_filenames.sort()
    now = datetime.datetime.now()
    print(now)
    pool = Pool(processes=8)  # Use 4 processes
    pool.map(process_file, filtered_filenames)  # Process the first 2 files
    pool.close()
    pool.join()
    now = datetime.datetime.now()
    print(now)
...
save_path = '/home/xhordurbhe/Verkefni/lamah_ice/modis_on_watersheds/timeseries/modis_glacier_albedo.csv'
